...
// 1. 检查是否传入了文件路径
   ...
   string filename_with_ext = filepath;
   string pure_name = filepath;
   // a. 去掉路径前缀 (找到最后一个 / 或 \)
   size_t last_slash_idx = filepath.find_last_of("/\\");
   if (std::string::npos != last_slash_idx) {
       filename_with_ext = filepath.substr(last_slash_idx + 1);
   }
   // b. 去掉扩展名 (找到最后一个 .)
   size_t last_dot_idx = filename_with_ext.find_last_of(".");
   if (std::string::npos != last_dot_idx) {
       pure_name = filename_with_ext.substr(0, last_dot_idx);
   } else {
       pure_name = filename_with_ext;
   }
   cout << "Input File: " << filepath << endl;
   cout << "Extracted Name: " << pure_name << endl;
   // 3. 构建输出文件名
   string output_name = pure_name; 
   output_name.append(".out"); // 结果: dre15_Output.out
   // 4. 打开文件并重定向 cin
   // 这一步非常关键：它让后续所有的 cin >> 操作都自动从文件读取，而不需要修改算法代码
   ifstream input_file(filepath.c_str());
   if (!input_file.is_open()) {
       cout << "Error: Could not open file " << filepath << endl;
       return 1;
   }
   // 保存旧的 cin buffer (可选，但在 main 结束时是个好习惯)
  ...
  target=0;
   // 读取第一个数字作为 N
    if (!(cin >> n)) {
        cout << "Error: Could not read N from file." << endl;
        return 0;
    }

    // [关键修复]: 吃掉第一行剩下的所有内容 (例如 " 3296" 或换行符)
    // 这样文件指针就会安全地移到第二行开头
    string dummy_line;
    getline(cin, dummy_line); 

    cout << "Problem Size (N): " << n << endl;
    // 如果 dummy_line 不为空，说明第一行确实有额外信息，可以打印出来看看
    if (dummy_line.length() > 1) {
        cout << "Ignored extra info in header: " << dummy_line << endl;
    }
