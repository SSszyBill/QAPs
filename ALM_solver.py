...
class ALM_QAP_Solver:
    def __init__(self, n, A_np, B_np, device='cpu', rho=10.0, batch_size=100):
        ...
        self.lam_row = torch.zeros((batch_size, n), device=device)
        self.lam_col = torch.zeros((batch_size, n), device=device)
    ...
    def solve(self, max_iter=2000, lr_dual=0.01):
        ...
        for t in range(max_iter):
            ...
            # 记录 QAP Cost (便于画图)
            cost_history.append(loss_qap.min().item())
            ...
            with torch.no_grad():
                ...
                # lambda <- lambda + rho * error
                # 注意 ALM 通常使用固定的 rho 更新步长，或者单独的学习率
