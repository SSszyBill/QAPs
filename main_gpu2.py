...
def grad_L_X_batch(F, D, X, Y, diag_F=None, diag_D=None):
    """
    ...
    # FtX = torch.matmul(F.t(), X)
    # FtXD = torch.matmul(FtX, D)
    obj_grad = grad_trace(F, D, X)  # (bs, n, n)
...
def obj_fn_batch(F, D, X, diag_F=None, diag_D=None):
    """
    trace(F * X * D.T * X.T)
    F, D: (n, n)
    """
...
def solve_torch_batch(instance, optimizer="adam",
    ...
    # extract the diagonal of each matrix in the batch, keep the same size as F and D
    diag_F = torch.diagonal(F, dim1=-2, dim2=-1)
    diag_D = torch.diagonal(D, dim1=-2, dim2=-1)
    
    # set the diagonal to zero
    F = F - torch.diag_embed(diag_F)
    D = D - torch.diag_embed(diag_D)
    ...
    for it in range(num_iters):
        # gradient steps
        grad_X = grad_L_X_batch(F, D, X, Y, diag_F, diag_D) # (bs, n, n)
        ...
        obj_vals = obj_fn_batch(F, D, X_int, diag_F, diag_D) # (bs,)
